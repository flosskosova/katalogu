...
3. **Session pooler** — Use Supabase **Connect → Session pooler**, host `*.pooler.supabase.com`, port **5432**; URL-encode special characters in the DB password. See `site/.env.example` (Postgres section). Session mode caps **concurrent** connections (often **15** per pool user). Each Vercel lambda runs its own `pg` pool: **`PAYLOAD_POSTGRES_POOL_MAX` defaults to 2** when `VERCEL=1` (Payload transactions can need more than one checkout; **`=1` hangs admin bulk delete** with “Deleting…”). Do not set `1` unless you accept that risk; stay **≤2** unless Supabase gives a higher session limit. This repo **patches** `@payloadcms/db-postgres` so the adapter releases its init `pool.connect()` listener client (upstream held it forever).
