...
3. **Session pooler** — Use Supabase **Connect → Session pooler**, host `*.pooler.supabase.com`, port **5432**; URL-encode special characters in the DB password. See `site/.env.example` (Postgres section). Session mode caps **concurrent** connections (often **15** per pool user). Each Vercel lambda runs its own `pg` pool: default **`max: 2`** when `VERCEL=1` (Payload **Accept/Delete** in admin need two checkouts inside a transaction). Do **not** set `PAYLOAD_POSTGRES_POOL_MAX=1` on Vercel unless you accept hung saves. For **local dev**, avoid pointing `DATABASE_URL` at production Supabase (or use local SQLite); otherwise you share the session cap and can hit **EMAXCONNSESSION**.
