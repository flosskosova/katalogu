...
3. **Session pooler** — Use Supabase **Connect → Session pooler**, host `*.pooler.supabase.com`, port **5432**; URL-encode special characters in the DB password. See `site/.env.example` (Postgres section). On Vercel the app uses a **small** `pg` pool (default **max 2**): Payload’s adapter keeps one client checked out after connect, so **`max: 1` starves queries** and you see `timeout exceeded when trying to connect` on `select count(*) from "users"`. Do not set `PAYLOAD_POSTGRES_POOL_MAX=1` in Production unless you know what you are doing; leave it unset or use `2`.
